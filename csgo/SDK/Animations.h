...
struct aimmatrix_transition_t
{
	...
	{
		auto Approach = [](float flTarget, float flValue, float _flSpeed) -> float
		{
			float flDelta = flTarget - flValue;

			if (flDelta > _flSpeed)
				flValue += _flSpeed;
			else if (flDelta < -_flSpeed)
				flValue -= _flSpeed;
			else
				flValue = flTarget;
...
{
	float GetValue(CPlayer* pPlayer);
	bool Init(CPlayer* pPlayer, const char* szName);
...
static_assert(FIELD_OFFSET(CCSGOPlayerAnimState, m_pPlayer) == 0x60, "Wrong struct offset");
static_assert(FIELD_OFFSET(CCSGOPlayerAnimState, m_flGoalFeetYaw) == 0x80, "Wrong struct offset");