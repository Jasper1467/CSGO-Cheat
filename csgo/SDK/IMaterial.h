...
enum MaterialPropertyType_e : int
{
...
public:
	ITexture* GetTexture()
	{
		return CallVFunc<ITexture*>(this, 1);
	}

	void SetValue(float flValue)
	{
		CallVFunc<void>(this, 4, flValue);
	}

	void SetValue(int iValue)
	{
		CallVFunc<void>(this, 5, iValue);
	}

	void SetValue(const char* szValue)
	{
		CallVFunc<void>(this, 6, szValue);
	}

	void SetValues(float x, float y)
	{
		CallVFunc<void>(this, 10, x, y);
	}

	void SetValues(float x, float y, float z)
	{
		CallVFunc<void>(this, 11, x, y, z);
	}

	void SetValues(const Vector& Val)
	{
		CallVFunc<void>(this, 11, Val.x, Val.y, Val.z);
	}

	void SetValues(const Color& Val)
	{
		CallVFunc<void>(this, 11, Val.Get<0>(), Val.Get<1>(), Val.Get<2>());
	}

	void SetTexture(ITexture* pTexture)
	{
		CallVFunc<void>(this, 15, pTexture);
	}

	void SetVectorComponent(float flValue, int iComponent)
	{
		CallVFunc<void>(this, 26, flValue, iComponent);
	}
};
...
public:
	...
	virtual PreviewImageRetVal_e GetPreviewImageProperties(int* iWidth, int* iHeight, ImageFormat_e* pImageFormat, bool* bTranslucent) const = 0;
	virtual PreviewImageRetVal_e GetPreviewImage(unsigned char* pData, int iWidth, int iHeight, ImageFormat_e imageFormat) const = 0;
	...
	virtual void			SetMaterialVarFlag(MaterialVarFlags_e flag, bool bEnable) = 0;
	virtual bool			GetMaterialVarFlag(MaterialVarFlags_e flag) = 0;
	virtual void			GetReflectivity(Vector& vecReflect) = 0;
	virtual bool			GetPropertyFlag(MaterialPropertyType_e type) = 0;
	...
	{
		ColorModulate(color.Get<0>() / 255, color.Get<1>() / 255, color.Get<2>() / 255);
		AlphaModulate(color.Get<3>());
	}
};